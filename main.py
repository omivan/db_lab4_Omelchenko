...
with conn:
    ...
    print('Кількість разів компанія була згадана як publisher або developer(топ 5 DESC)')
    for row in cur:
        print(f'Компанія: {row[0]}, Кількість разів: {row[1]}')

    cur.execute(query_2)

    print('\nЧастка кожного жанру в іграх(топ 5 DESC)')
    for row in cur:
        print(f'Жанр: {row[0]}, Кількість ігор: {row[1]}')

    cur.execute(query_3)

    print('\nТоп 5 компанії за кількістю користувачів')
    for row in cur:
